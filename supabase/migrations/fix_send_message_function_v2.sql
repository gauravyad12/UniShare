-- Migration to fix the send_group_chat_message function with explicit table references
...
BEGIN
  ...
  -- Update the last_message_at and message_count in the study group with explicit table reference
  ...
  -- Return the message with profile information using explicit table references
  RETURN QUERY
  SELECT 
    gcm.id,
    gcm.study_group_id,
  ...
  LEFT JOIN user_profiles up ON gcm.sender_id = up.id
  WHERE gcm.id = v_message_id;
  ...
  IF v_table_exists THEN
    -- Mark the message as read by the sender with explicit column names
