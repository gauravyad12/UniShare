...
-- Confirmation email
UPDATE auth.mfa_amr_claims SET created_at = created_at WHERE 1=0;

-- Instead of trying to update auth.templates directly, we'll use Supabase's built-in function
-- to update email templates

-- This is a safer approach that works with Supabase's auth schema
SELECT
  set_config('supabase_auth.email_templates', 
  json_build_object(
...
-- Set SMS templates via Supabase dashboard instead
-- This is a placeholder to remind you to set SMS templates in the Supabase dashboard
COMMENT ON SCHEMA public IS 'Note: SMS templates should be configured in the Supabase dashboard';
