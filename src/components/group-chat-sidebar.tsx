...
export default function GroupChatSidebar({ currentGroupId }: GroupChatSidebarProps) {
  ...
  const fetchGroups = async () => {
    try {
      ...
      console.log('Raw groups data before processing:', JSON.stringify(groupsData));
      ...
      try {
        ...
        console.log("First group sample:", groupsWithLatestMessage[0] ? JSON.stringify(groupsWithLatestMessage[0]) : 'No groups');

        // Check if current group is in the list, if not, fetch and add it
        if (currentGroupId && !groupsWithLatestMessage.some(g => g.id === currentGroupId)) {
          console.log('Current group not in list, fetching it separately:', currentGroupId);
          ...
          if (currentGroupError) {
            console.error('Error fetching current group:', currentGroupError);
          } else if (currentGroup) {
            console.log('Successfully fetched current group:', currentGroup);

            // Get latest message for current group
            ...
            groupsWithLatestMessage.push(groupWithMessage);
          }
        }
      ...
      console.log("User membership status being checked");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    console.log('GroupChatSidebar mounted, fetching groups...');
    // Initial fetch
    fetchGroups();

    // Set up realtime subscription for study_group_members
    const membershipChannel = supabase
      .channel('group-chat-sidebar-membership')
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
      ...
      .subscribe((status) => {
        console.log('Membership subscription status:', status);
      });
    ...
    return () => {
      console.log('GroupChatSidebar unmounting, removing channels...');
      supabase.removeChannel(membershipChannel);
      supabase.removeChannel(messagesChannel);
    };
  }, []);

  // No need to filter since we're only showing the current group
