...
export async function POST(request: NextRequest) {
  try {
    ...
    if (recordingsWithTranscripts.length === 0) {
      ...
      return NextResponse.json(
        { 
          ...
          message: `The recording "${recordingTitles}" doesn't have a transcript. To generate a summary, please record a new lecture with speech or manually add a transcript to your recording.`,
          details: 'Summary generation requires transcript content to analyze and summarize your lecture content.'
        },
        { status: 400 }
      );
    }
    ...
    console.error('Lecture summary generation error:', error);
    return NextResponse.json(
