...
export async function POST(request: NextRequest) {
  try {
    ...
    const { recordingIds, style = 'structured' } = await request.json();
    ...
    return NextResponse.json({ 
      ...
      jobId: job.id,
      status: 'pending',
      message: 'Notes generation started. Please check status using the job ID.'
    });
