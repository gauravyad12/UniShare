...
import { Check, X, Sparkles, BookMarked, Globe, BarChart3, FileText, MessageSquare, Mic, ArrowLeft } from "lucide-react";
import Link from "next/link";
import { createClient } from "@/utils/supabase/client";
import { User } from "@supabase/supabase-js";

export default function PricingClientWithData() {
  const [user, setUser] = useState<User | null>(null);
  const [pricingPlans, setPricingPlans] = useState<any[]>([]);
  const [hasActiveSubscription, setHasActiveSubscription] = useState(false);
  const [dataLoaded, setDataLoaded] = useState(false);

  // Define fallback plans (same as original)
  const fallbackPlans = [
    {
      id: "price_free",
      name: "Free",
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Check for active subscription if user is logged in
        if (currentUser) {
          try {
            ...
            if (!subscriptionError && subscriptions && subscriptions.length > 0) {
              // Check if the most recent subscription is still valid
              const currentTime = Math.floor(Date.now() / 1000);
              const latestSubscription = subscriptions[0];

              const isValid = latestSubscription.status === "active" &&
        ...
        // Use fallback plans to avoid any visual changes
        // The fallback plans contain the correct pricing and features
  ...
  const displayPlans = pricingPlans.length > 0 ? pricingPlans : fallbackPlans;

  // Show redirect message for users with active subscriptions
  if (dataLoaded && hasActiveSubscription) {
    return (
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="container mx-auto px-4 py-8 md:py-16 flex-1">
        ...
        {/* Pricing cards - using original PricingCard component */}
